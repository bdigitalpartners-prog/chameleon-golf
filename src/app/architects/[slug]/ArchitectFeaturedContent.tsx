...
export function ArchitectFeaturedContent({
  architectId,
  architectName,
}: {
  architectId: number;
  ...
  return (
    ...
    >
      ...
      >
        Featured Content about {architectName}
      </h2>
      <div className="grid gap-3 sm:grid-cols-2 lg:grid-cols-3">
        {items.map((item) => {
          const Icon = TYPE_ICONS[item.contentType] || FileText;
          const colors = TYPE_COLORS[item.contentType] || { bg: "var(--cg-bg-secondary)", text: "var(--cg-text-muted)" };
          return (
            <a
              key={item.id}
              href={item.url}
            ...
            >
              <div className="flex items-center gap-2 mb-2">
                ...
                <div className="flex-1 min-w-0">
                  ...
                  {item.sourceName && (
                    <span className="text-[10px] ml-1.5" style={{ color: "var(--cg-text-muted)" }}>
                      {item.sourceName}
                    </span>
                  )}
                </div>
