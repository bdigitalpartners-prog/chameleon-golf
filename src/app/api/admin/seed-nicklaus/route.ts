/**
 * Temporary API endpoint to run the Jack Nicklaus course seed script.
 * Remove after successful execution.
 *
 * POST /api/admin/seed-nicklaus
 */
...
export async function POST(request: NextRequest) {
  ...
  try {
    log('Starting Jack Nicklaus Course Ingestion...');

    // Step 1: Upsert Architects
    log('Step 1: Upserting architects...');
    ...
    for (const arch of architects) {
      ...
      log(`  Architect: ${arch.name} (ID: ${result.id})`);
    }

    // Step 2: Upsert Courses
    log('Step 2: Upserting courses...');
    let created = 0;
    let updated = 0;
    let errors = 0;
    ...
    for (const course of topCourses) {
      try {
        ...
        if (existing) {
          ...
          updated++;
          log(`  Updated: ${course.courseName} (ID: ${courseId})`);
        } else {
          ...
          courseId = result.courseId;
          created++;
          log(`  Created: ${course.courseName} (ID: ${courseId})`);
        }

        // Link to architects via CourseArchitect junction
        ...
        errors++;
        log(`  Error with ${course.courseName}: ${err.message}`);
      }
    }

    // Summary
    const summary = {
      architectsUpserted: architects.length,
      coursesCreated: created,
      coursesUpdated: updated,
      errors,
      totalProcessed: topCourses.length,
    };
    log(`COMPLETE: ${JSON.stringify(summary)}`);
