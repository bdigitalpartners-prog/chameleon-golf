...
import {
  ...
  ArrowRight, Footprints, ShoppingCart, User,
} from "lucide-react";
...
interface CompareCourse {
  ...
  dna: {
    ...
    bunkerChallenge: number;
    waterInfluence: number;
    elevationChange: number;
    windExposure: number;
    recoveryDifficulty: number;
    lengthChallenge: number;
    walkabilityScore: number;
    conditioningStandard: number;
  } | null;
  tournaments: { tournamentName: string; tour: string; year: number; winnerName: string; winnerScore: string }[];
  tournamentCount: number;
  latestCondition: {
    greenSpeed: string | null;
    greenFirmness: string | null;
    fairwayCondition: string | null;
...
interface PopularComparison {
  ...
  description: string;
  ids: number[];
  courses: { courseId: number; courseName: string; chameleonScore: number | null }[];
}
...
const DNA_DIMENSIONS = [
  { key: "shotVariety", label: "Shot Variety" },
  { key: "strategicOptions", label: "Strategic Options" },
  { key: "visualDrama", label: "Visual Drama" },
  { key: "greenComplexity", label: "Green Complexity" },
  { key: "bunkerChallenge", label: "Bunker Challenge" },
  { key: "waterInfluence", label: "Water Influence" },
  { key: "elevationChange", label: "Elevation Change" },
  { key: "windExposure", label: "Wind Exposure" },
  { key: "recoveryDifficulty", label: "Recovery Difficulty" },
  { key: "lengthChallenge", label: "Length Challenge" },
  { key: "walkabilityScore", label: "Walkability" },
  { key: "conditioningStandard", label: "Conditioning" },
] as const;

const COURSE_COLORS = ["#00FF85", "#3B82F6", "#F59E0B", "#EF4444"];

/* ─── Walking Policy Icons ─── */
function WalkingPolicyDisplay({ policy }: { policy: string | null }) {
  if (!policy) return <span style={{ color: "var(--cg-text-muted)" }}>N/A</span>;
  const lower = policy.toLowerCase();
  const isWalkable = lower.includes("walk") || lower.includes("yes");
  const isCartRequired = lower.includes("cart required") || lower.includes("mandatory cart");
  const hasCaddie = lower.includes("caddie");

  return (
    <div className="flex items-center gap-1.5">
      {isWalkable && <Footprints className="h-3.5 w-3.5" style={{ color: "var(--cg-accent)" }} />}
      {isCartRequired && <ShoppingCart className="h-3.5 w-3.5" style={{ color: "#f59e0b" }} />}
      {hasCaddie && <User className="h-3.5 w-3.5" style={{ color: "#3b82f6" }} />}
      <span className="text-xs">{policy}</span>
    </div>
  );
}

/* ─── Radar Chart (overlay — supports both 9-dim and 12-dim DNA) ─── */
...
function OverlayRadarChart({
  ...
  getValues: (course: CompareCourse) => Record<string, number | null>;
  maxValue?: number;
  title: string;
}) {
  ...
  const polygonPoints = (vals: Record<string, number | null>) => {
    return dimensions
      .map((d, i) => {
        const val = vals[d.key] ?? 0;
        const pt = getPoint(i, (val / maxValue) * r);
        return `${pt.x},${pt.y}`;
      })
      .join(" ");
  };

  const hasAnyData = courses.some((c) => {
    const vals = getValues(c);
    return dimensions.some((d) => vals[d.key] != null);
  });

  if (!hasAnyData) {
    return (
      <div className="flex items-center justify-center py-8" style={{ color: "var(--cg-text-muted)" }}>
    ...
    <div className="flex flex-col items-center">
      <svg viewBox={`0 0 ${size} ${size}`} className="w-full max-w-[340px]">
        ...
        {dimensions.map((dim, i) => {
          ...
          const labelText = dim.label.length > 10 ? dim.label.split(" ")[0] : dim.label;
          return (
            <text
              key={dim.key}
              x={pt.x}
...
function ComparePageContent() {
  ...
  const courseIds = useMemo(() => {
    const param = searchParams.get("courses") || searchParams.get("ids");
    if (!param) return [];
    return param
      .split(",")
  ...
  useEffect(() => {
    fetch("/api/compare/popular")
      .then((r) => r.json())
      .then((data) => setPopularComparisons(data.comparisons ?? []))
      .catch(() => {});
  }, []);
  ...
  // Collect all unique ranking sources
  ...
  const summary = useMemo(() => {
    ...
    DIMENSIONS.forEach((dim) => {
      const vals = courses.map(
        (c) => (c.dimensions?.[dim.key as keyof CompareCourse["dimensions"] & string] as number | null) ?? null
      );
    ...
    const totalCategories = 1 + DIMENSIONS.length + 1;
  ...
  const hasDna = courses.some((c) => c.dna != null);
  const hasTournaments = courses.some((c) => c.tournamentCount > 0);
  const hasConditions = courses.some((c) => c.latestCondition != null);
      ...
      <div className="mx-auto max-w-7xl px-4 py-8">
        ...
        <div className="mb-8">
          ...
          <p className="mt-2 text-base" style={{ color: "var(--cg-text-secondary)" }}>
            Multi-axis side-by-side comparison — scores, DNA fingerprints, green fees, walking policies, and more.
          </p>
        </div>
        ...
        {!loading && !hasComparison && (
          ...
          >
            ...
            <p className="text-sm mb-6" style={{ color: "var(--cg-text-muted)" }}>
              Search above to add 2-4 courses, or try a popular comparison below.
            </p>
            ...
            {popularComparisons.length > 0 ? (
              <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-3 text-left">
                {popularComparisons.map((p) => (
                  ...
                  >
                    <div className="text-sm font-bold mb-1" style={{ color: "var(--cg-text-primary)" }}>
                      {p.label}
                    </div>
                    <div className="text-xs" style={{ color: "var(--cg-text-muted)" }}>
                      {p.description}
                    ...
                    <div className="flex items-center gap-1 mt-2">
                      <ArrowRight className="h-3 w-3" style={{ color: "var(--cg-accent)" }} />
        ...
        {!loading && hasComparison && (
          <div className="space-y-6">
            ...
            <div className="grid gap-4" style={{ gridTemplateColumns: `repeat(${courses.length}, 1fr)` }}>
                ...
                >
                  <div className="relative aspect-[16/9]" style={{ backgroundColor: "var(--cg-bg-tertiary)" }}>
                    {course.primaryImageUrl ? (
                      <img src={course.primaryImageUrl} alt={course.courseName} className="h-full w-full object-cover" />
                    ) : (
                    ...
                    {course.chameleonScore != null && (
                      <div
                        ...
                        style={{
                          backgroundColor: course.chameleonScore >= 80 ? "var(--cg-accent)" : course.chameleonScore >= 50 ? "#eab308" : "var(--cg-bg-tertiary)",
                          color: course.chameleonScore >= 50 ? "var(--cg-text-inverse)" : "var(--cg-text-secondary)",
                        }}
                      >
                  ...
                  <div className="p-4">
                    ...
                    {course.accessType && (
                      <span
                        className="inline-block mt-1 rounded-md px-2 py-0.5 text-[10px] font-medium"
              ...
              {courses.length < 4 && (
                <div
                  ...
                  style={{ backgroundColor: "var(--cg-bg-card)", border: "2px dashed var(--cg-border)" }}
                  onClick={() => {
                    const input = document.querySelector<HTMLInputElement>('input[placeholder*="Search courses"]');
                    input?.focus();
                  }}
                ...
                >
                  ...
                  <span className="text-sm font-medium" style={{ color: "var(--cg-text-muted)" }}>Add Course</span>
                </div>
              )}
            </div>
            ...
            <div
              ...
              style={{ backgroundColor: "var(--cg-bg-card)", border: "1px solid var(--cg-border)" }}
            >
              <h2 className="font-display text-lg font-bold mb-4" style={{ color: "var(--cg-text-primary)" }}>
                9-Dimension Score Comparison
              </h2>
              <OverlayRadarChart
                courses={courses}
                dimensions={DIMENSIONS}
                getValues={(c) => (c.dimensions || {}) as any}
                maxValue={10}
                title="9-Dimension Scores"
              />
            </div>

            {/* Course DNA Radar (if data exists) */}
            {hasDna && (
              <div
                className="rounded-xl p-6"
                style={{ backgroundColor: "var(--cg-bg-card)", border: "1px solid var(--cg-border)" }}
              >
                <h2 className="font-display text-lg font-bold mb-4" style={{ color: "var(--cg-text-primary)" }}>
                  Course DNA Fingerprint
                </h2>
                <OverlayRadarChart
                  courses={courses}
                  dimensions={DNA_DIMENSIONS}
                  getValues={(c) => (c.dna || {}) as any}
                  maxValue={100}
                  title="Course DNA"
                />
              </div>
                  ...
                  {courses.map((course, ci) => (
                    <div key={course.courseId}>
                      <div
                        ...
                        style={{ color: COURSE_COLORS[ci % COURSE_COLORS.length] }}
                      >
                        {course.tournamentCount} tournament{course.tournamentCount !== 1 ? "s" : ""}
                      </div>
                      {course.tournaments.length > 0 ? (
                        <div className="space-y-1.5">
                          {course.tournaments.slice(0, 5).map((t, ti) => (
                            <div
                              key={ti}
                            ...
                            >
                              ...
                              {t.winnerName && (
                                <div style={{ color: "var(--cg-text-muted)" }}>
                                  Winner: {t.winnerName} {t.winnerScore && `(${t.winnerScore})`}
                                </div>
                              )}
                            </div>
                          ))}
            ...
            {hasConditions && (
              ...
              >
                ...
                <div className="grid gap-4" style={{ gridTemplateColumns: `repeat(${courses.length}, 1fr)` }}>
                  {courses.map((course, ci) => {
                    const cond = course.latestCondition;
                    return (
                      <div key={course.courseId}>
                        <div
                          className="text-sm font-bold mb-2"
                        ...
                        {cond ? (
                          <div className="space-y-1 text-xs">
                            {cond.greenSpeed && (
                              <div>
                                <span style={{ color: "var(--cg-text-muted)" }}>Greens: </span>
                                <span style={{ color: "var(--cg-text-primary)" }}>{cond.greenSpeed}</span>
                              </div>
                            )}
                            {cond.fairwayCondition && (
                              <div>
                                <span style={{ color: "var(--cg-text-muted)" }}>Fairways: </span>
                                <span style={{ color: "var(--cg-text-primary)" }}>{cond.fairwayCondition}</span>
                              </div>
                            )}
                            {cond.overallRating && (
                              <div>
                                <span style={{ color: "var(--cg-text-muted)" }}>Rating: </span>
                                <span style={{ color: "var(--cg-accent)" }}>{"★".repeat(cond.overallRating)}</span>
                              </div>
                            )}
                          </div>
                        ) : (
                          <span className="text-xs" style={{ color: "var(--cg-text-muted)" }}>No condition data</span>
                        )}
                      </div>
                    );
                  })}
            ...
            >
              <div className="overflow-x-auto">
                <table className="w-full min-w-[600px]">
                  <thead>
                    <tr style={{ borderBottom: "1px solid var(--cg-border)" }}>
                      <th
                        ...
                        style={{ color: "var(--cg-text-muted)", backgroundColor: "var(--cg-bg-card)" }}
                      >
                        Attribute
                      </th>
                      {courses.map((course, i) => (
                  ...
                  <tbody>
                    {/* Overview */}
                    <SectionHeader label="Overview" colSpan={courses.length + 1} />
                    ...
                    <CompareRow label="Yardage" values={courses.map((c) => c.yardage != null ? c.yardage.toLocaleString() : null)} />
                    ...
                    <CompareRow label="Location" values={courses.map((c) => [c.city, c.state, c.country].filter(Boolean).join(", ") || null)} />

                    {/* Rankings & Scores */}
                    <SectionHeader label="Rankings & Scores" colSpan={courses.length + 1} />
                    <CompareRowHighlight label="EQ Score" values={courses.map((c) => c.chameleonScore)} format={(v) => Math.round(v).toString()} mode="high" />
                    <CompareRow label="Prestige" values={courses.map((c) => c.prestigeScore != null ? Number(c.prestigeScore).toFixed(1) : null)} />
                    <CompareRow label="Ratings" values={courses.map((c) => c.totalRatings > 0 ? String(c.totalRatings) : null)} />
                    ...
                    {allRankingSources.map((source) => (
                      <CompareRow
                        ...
                        values={courses.map((c) => {
                          const r = c.rankings.find((r) => `${r.source} — ${r.list}` === source);
                    ...
                    {DIMENSIONS.map((dim) => (
                      <CompareRowHighlight
                        key={dim.key}
                        label={dim.label}
                        values={courses.map((c) => (c.dimensions?.[dim.key as keyof CompareCourse["dimensions"] & string] as number | null) ?? null)}
                    ...
                    <CompareRowHighlight label="Standard" values={courses.map((c) => c.greenFeeLow)} format={(v) => formatCurrency(v)} mode="low" />
                    <CompareRow label="Peak" values={courses.map((c) => c.greenFeeHigh != null ? formatCurrency(c.greenFeeHigh) : c.greenFeePeak != null ? formatCurrency(c.greenFeePeak) : null)} />
                    <CompareRow label="Twilight" values={courses.map((c) => c.greenFeeTwilight != null ? formatCurrency(c.greenFeeTwilight) : null)} />

                    {/* Walking & Access */}
                    <SectionHeader label="Walking & Access" colSpan={courses.length + 1} />
                    <tr style={{ borderBottom: "1px solid var(--cg-border-subtle)" }}>
                      <td className="sticky left-0 z-10 px-5 py-2.5 text-xs font-semibold" style={{ color: "var(--cg-text-muted)", backgroundColor: "var(--cg-bg-card)" }}>
                        Walking Policy
                      </td>
                      {courses.map((c) => (
                        <td key={c.courseId} className="px-4 py-2.5 text-sm" style={{ color: "var(--cg-text-primary)" }}>
                          <WalkingPolicyDisplay policy={c.walkingPolicy} />
                        </td>
                      ))}
                    </tr>
                    ...
                    {/* Architect */}
                    <SectionHeader label="Architect" colSpan={courses.length + 1} />
                    <CompareRow
                      label="Designer"
                      values={courses.map((c) => c.architect ? c.architect.name : c.originalArchitect ?? null)}
                    />
                    <CompareRow
                      label="Era"
                      values={courses.map((c) => {
                        const y = c.yearOpened;
                        if (!y) return null;
                        if (y < 1930) return "Golden Age";
                        if (y < 1960) return "Classic";
                        if (y < 1990) return "Modern";
                        return "Contemporary";
                      })}
                    />
                    ...
                    <CompareRowHighlight label="Slope Rating" values={courses.map((c) => c.slopeRating)} format={(v) => v.toString()} mode="high" />
                    <CompareRow label="Course Rating" values={courses.map((c) => c.courseRating != null ? c.courseRating.toFixed(1) : null)} />

                    {/* Tournament Heritage */}
                    <SectionHeader label="Tournament Heritage" colSpan={courses.length + 1} />
                    <CompareRowHighlight label="Tournament Count" values={courses.map((c) => c.tournamentCount)} format={(v) => String(v)} mode="high" />
                    <CompareRow
                      label="Most Recent"
                      values={courses.map((c) => {
                        const t = c.tournaments[0];
                        return t ? `${t.tournamentName} (${t.year})` : null;
                      })}
                    />
                    ...
                    <CompareRow label="Best Time" values={courses.map((c) => c.bestTimeToPlay)} />
                    <CompareRow label="Best Months" values={courses.map((c) => c.bestConditionMonths)} />
                  </tbody>
                </table>
              </div>
            </div>

            {/* Popular Comparisons at bottom */}
            {popularComparisons.length > 0 && (
              <div>
                <h2 className="font-display text-lg font-bold mb-4" style={{ color: "var(--cg-text-primary)" }}>
                ...
                <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-3">
                  {popularComparisons
                    .filter((p) => !p.ids.every((id) => courseIds.includes(id)))
                    .slice(0, 6)
                    .map((p) => (
                      <button
                        key={p.label}
    ...
    <tr style={{ borderBottom: "1px solid var(--cg-border-subtle)" }}>
      ...
      {values.map((v, i) => (
        <td
          ...
          style={{
            color: v != null && i === bestIdx ? "var(--cg-accent)" : v != null ? "var(--cg-text-primary)" : "var(--cg-text-muted)",
        ...
        >
          {v != null ? (
            <>
              {format(v)}
              {i === bestIdx && (
                <Trophy className="inline h-3 w-3 ml-1" style={{ color: "var(--cg-accent)" }} />
              )}
            </>
          ) : (
            "N/A"
