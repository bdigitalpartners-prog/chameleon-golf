  ...
  const handleKeyDown = (e: React.KeyboardEvent) => {
    if (e.key === "ArrowDown") {
      ...
      if (selectedIndex >= 0 && results[selectedIndex]) {
        navigateTo(results[selectedIndex].courseId);
      }
    ...
    >
      ...
      >
                    ...
                    <div className="flex items-center gap-2 text-sm" style={{ color: "var(--cg-text-muted)" }}>
                      ...
                      {r.originalArchitect && (
                        <>
                          ...
                          <span className="truncate">{r.originalArchitect}</span>
                        </>
                      )}
                    </div>
                  </div>
        ...
        {query.length >= 2 && results.length === 0 && !loading && (
          <div className="px-4 py-6 text-center text-sm" style={{ color: "var(--cg-text-muted)" }}>
            No courses found for &ldquo;{query}&rdquo;
          </div>
        )}
        {query.length >= 2 && results.length === 0 && !loading && (
          <div className="px-4 pb-4 text-center text-xs" style={{ color: "var(--cg-text-muted)", opacity: 0.6 }}>
            Try a course name, city, state, or architect
          </div>
        )}
      </div>
    </div>
